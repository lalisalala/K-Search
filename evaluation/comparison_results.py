...
def compute_bertscore(relevant_texts, retrieved_texts):
    if not retrieved_texts or not relevant_texts:
        return 0, 0, 0  # No valid comparison
    ...
    try:
        ...
        P, R, F1 = score(retrieved_texts, relevant_texts, model_type="bert-base-uncased", lang="en", rescale_with_baseline=True, device=device)
...
for i, metric in enumerate(metrics):
    ax = axes[i]
    ax.bar(x - width, keyword_bert_results[metric], width=width, label="Keyword", color=colors["Keyword"])
    ax.bar(x, sparql_bert_results[metric], width=width, label="SPARQL", color=colors["SPARQL"])
    ax.bar(x + width, faiss_bert_results[metric], width=width, label="FAISS", color=colors["FAISS"])
